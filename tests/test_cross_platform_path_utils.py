...
@pytest.mark.skipif(
    os.name == "nt", reason="foreign Windows path behavior is only relevant off Windows"
)
def test_foreign_windows_paths_are_not_resolved_as_posix_paths() -> None:
    ...
    assert (
        join_path_string(data_dir, "papers", "paper.pdf") == r"D:\ResearchOS\data\papers\paper.pdf"
    )
    assert (
        sqlite_url_for_path(r"D:\ResearchOS\data\researchos.db")
        == "sqlite:///D:/ResearchOS/data/researchos.db"
    )
...
def test_foreign_windows_relative_paths_use_windows_boundaries() -> None:
    ...
    assert (
        local_relative_path(workspace, r"D:\ResearchOS\data\workspace\reports\out.md")
        == "reports/out.md"
    )
    assert (
        local_relative_path(workspace, r"d:\researchos\data\workspace\reports\..\out.md")
...
def test_posix_style_windows_drive_paths_keep_posix_separators() -> None:
    ...
    assert path_name_string(data_dir) == "data"
    assert (
        join_path_string(data_dir, "papers", "paper.pdf") == "D:/ResearchOS/data/papers/paper.pdf"
    )
...
def test_unc_windows_sqlite_url_uses_sqlite_unc_form() -> None:
    assert (
        sqlite_url_for_path(r"\\server\share\researchos.db")
        == "sqlite://server/share/researchos.db"
    )
