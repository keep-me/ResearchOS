...
def format_idea_discovery_report(
    ...
    if created_ideas:
        executive_summary_lines.append(f"- 推荐优先推进：{str(created_ideas[0].get('title') or 'Top idea').strip()}。")
    ...
    if str(novelty_markdown or "").strip():
        lines.extend(["", "## Deep Novelty Verification", sanitize_project_markdown(novelty_markdown)])
    if str(review_markdown or "").strip():
        lines.extend(["", "## External Critical Review", sanitize_project_markdown(review_markdown)])
    lines.extend(
        [
            "",
            "## Next Steps",
...
def format_experiment_report(
    ...
    effective_command = _clean_text(execution.get("effective_command")) or _clean_text(payload.get("effective_execution_command"))
    workspace = _clean_text(execution.get("workspace_path")) or _clean_text(payload.get("execution_workspace"))
    remote_session = _clean_text(execution.get("remote_session_name")) or _clean_text(payload.get("remote_session_name"))
    remote_workspace = _clean_text(execution.get("remote_execution_workspace")) or _clean_text(payload.get("remote_execution_workspace"))
    remote_isolation_mode = _clean_text(execution.get("remote_isolation_mode")) or _clean_text(payload.get("remote_isolation_mode"))
    launch_status = _clean_text(payload.get("remote_launch_status")) or _clean_text(execution.get("mode"))
...
def format_paper_writing_report(
    ...
    payload = dict(metadata or {})
    stage_outputs = payload.get("stage_outputs") if isinstance(payload.get("stage_outputs"), dict) else {}
    ...
    polish_payload = stage_outputs.get("polish_manuscript") if isinstance(stage_outputs.get("polish_manuscript"), dict) else {}
    final_draft = sanitize_project_markdown(final_manuscript_markdown) or _stage_content(stage_outputs, "polish_manuscript", "draft_sections")
    venue = _resolve_paper_venue(payload)
    scores = payload.get("paper_improvement_scores") if isinstance(payload.get("paper_improvement_scores"), dict) else {}
    verdicts = payload.get("paper_improvement_verdicts") if isinstance(payload.get("paper_improvement_verdicts"), dict) else {}
    ...
    verdict_round_one = _clean_text(verdicts.get("round_1")) or _clean_text(polish_payload.get("verdict_round_one"))
    verdict_round_two = _clean_text(verdicts.get("round_2")) or _clean_text(polish_payload.get("verdict_round_two"))
    ...
    if score_round_one is not None or verdict_round_one:
        lines.append(f"- Round 1: score=`{score_round_one if score_round_one is not None else 'N/A'}`, verdict=`{verdict_round_one or 'N/A'}`")
    ...
    if score_round_two is not None or verdict_round_two:
        lines.append(f"- Round 2: score=`{score_round_two if score_round_two is not None else 'N/A'}`, verdict=`{verdict_round_two or 'N/A'}`")
    if action_items_round_two:
        lines.extend([f"- Round 2 action: {item}" for item in action_items_round_two[:6]])
    if not action_items_round_one and not action_items_round_two and score_round_one is None and score_round_two is None:
...
def format_paper_plan_report(
    ...
    venue = _resolve_paper_venue(payload)
    template_name = _clean_text(payload.get("paper_template")) or _clean_text(payload.get("template_name"))
...
def format_paper_compile_report(
    ...
    payload = dict(metadata or {})
    stage_outputs = payload.get("stage_outputs") if isinstance(payload.get("stage_outputs"), dict) else {}
    compile_stage = stage_outputs.get("run_compile") if isinstance(stage_outputs.get("run_compile"), dict) else {}
    ...
    pdf_paths = payload.get("compiled_pdf_paths") if isinstance(payload.get("compiled_pdf_paths"), list) else []
...
def format_paper_improvement_report(
    ...
    scores = payload.get("paper_improvement_scores") if isinstance(payload.get("paper_improvement_scores"), dict) else {}
    verdicts = payload.get("paper_improvement_verdicts") if isinstance(payload.get("paper_improvement_verdicts"), dict) else {}
    action_items = payload.get("paper_improvement_action_items") if isinstance(payload.get("paper_improvement_action_items"), dict) else {}
...
def format_full_pipeline_report(
    ...
    cleaned_handoff = sanitize_project_markdown(handoff_markdown) or _stage_content(stage_outputs, "handoff_output")
    execution = payload.get("execution_result") if isinstance(payload.get("execution_result"), dict) else {}
    command = _clean_text(execution.get("command")) or _clean_text(payload.get("execution_command"))
    effective_command = _clean_text(execution.get("effective_command")) or _clean_text(payload.get("effective_execution_command"))
    workspace = _clean_text(execution.get("workspace_path")) or _clean_text(payload.get("execution_workspace"))
    ...
    summary_lines.extend(_summary_bullets(cleaned_handoff or findings_markdown or review_markdown, max_items=4))
...
def format_rebuttal_report(
    ...
    cleaned_final = sanitize_project_markdown(final_markdown) or _stage_content(stage_outputs, "finalize_package")
    ...
    quick_mode = bool(
        ...
        or str(payload.get("rebuttal_quick_mode") or "").strip().lower() in {"1", "true", "yes", "on"}
    )
    ...
    if quick_mode:
        summary_lines.append("- 当前运行使用 quick mode，只输出 issue board 与 strategy plan，不生成最终提交稿。")
    summary_lines.extend(_summary_bullets(cleaned_final or strategy_markdown or issue_board_markdown, max_items=4))
...
def format_auto_review_loop_report(
    ...
    execution_command = _clean_text(latest_execution.get("command")) or _clean_text(payload.get("execution_command"))
    ...
    if iterations:
        for item in iterations:
            ...
            if round_score is not None or round_verdict:
                lines.append(f"- 评审结果: score=`{round_score if round_score is not None else 'N/A'}`, verdict=`{round_verdict or 'N/A'}`")
            if round_summary:
            ...
            execution_summary = sanitize_project_markdown(str(item.get("execution_summary") or "").strip())
...
def format_monitor_experiment_report(
    ...
    tracked_session_names = [str(item).strip() for item in (payload.get("remote_session_names") or []) if str(item).strip()]
...
def format_experiment_audit_report(
    ...
    payload = dict(metadata or {})
    raw_markdown = sanitize_project_markdown(str(payload.get("workflow_output_markdown") or "").strip())
    ...
    lines.append(f"**Overall Verdict**: {str(audit_payload.get('overall_verdict') or 'WARN').upper()}")
    lines.append(f"**Integrity Status**: {str(audit_payload.get('integrity_status') or 'warn').lower()}")
...
def format_sync_workspace_report(
    ...
    target_server = _clean_text(payload.get("target_workspace_server_id")) or _clean_text(payload.get("workspace_server_id")) or "local"
...
def format_custom_run_report(
    ...
    execution = payload.get("execution_result") if isinstance(payload.get("execution_result"), dict) else {}
    cleaned_body = sanitize_project_markdown(body)
    command = _clean_text(execution.get("command")) or _clean_text(payload.get("execution_command")) or _clean_text(payload.get("command"))
    workspace = _clean_text(execution.get("workspace_path")) or _clean_text(payload.get("execution_workspace"))
    exit_code = execution.get("exit_code")
...
def build_workflow_report_markdown(
    *,
    ...
    if workflow_key == "literature_review":
        body = _stage_content(stage_outputs, "deliver_review", "synthesize_evidence") or raw_markdown
    ...
    if workflow_key == "novelty_check":
        ...
        return format_novelty_check_report(project_label, prompt, comparison_markdown, verdict_markdown) if verdict_markdown else None
    ...
    if workflow_key == "research_review":
        ...
        return format_research_review_report(project_label, prompt, review_markdown, verdict_markdown) if verdict_markdown else None

    if workflow_key == "run_experiment":
        summary_markdown = _stage_content(stage_outputs, "summarize_results") or raw_markdown
        return format_experiment_report(project_label, prompt, summary_markdown, payload) if summary_markdown or payload.get("execution_result") else None

    if workflow_key == "experiment_audit":
        return format_experiment_audit_report(project_label, prompt, payload) if raw_markdown or stage_outputs or payload.get("audit_payload") else None

    if workflow_key == "paper_writing":
        final_manuscript_markdown = _stage_content(stage_outputs, "polish_manuscript", "draft_sections") or raw_markdown
        return format_paper_writing_report(project_label, prompt, final_manuscript_markdown, payload) if final_manuscript_markdown or stage_outputs else None

    if workflow_key == "rebuttal":
        final_rebuttal_markdown = _stage_content(stage_outputs, "finalize_package", "draft_rebuttal") or raw_markdown
        return format_rebuttal_report(project_label, prompt, final_rebuttal_markdown, payload) if final_rebuttal_markdown or stage_outputs else None

    if workflow_key == "paper_plan":
        plan_markdown = _stage_content(stage_outputs, "outline_manuscript", "collect_materials") or raw_markdown
        return format_paper_plan_report(project_label, prompt, plan_markdown, payload) if plan_markdown or stage_outputs else None

    if workflow_key == "paper_figure":
        figure_markdown = _stage_content(stage_outputs, "design_figures", "collect_results") or raw_markdown
        return format_paper_figure_report(project_label, prompt, figure_markdown, payload) if figure_markdown or stage_outputs else None

    if workflow_key == "paper_write":
        draft_markdown = _stage_content(stage_outputs, "draft_sections", "gather_materials") or raw_markdown
        return format_paper_write_report(project_label, prompt, draft_markdown, payload) if draft_markdown or stage_outputs else None

    if workflow_key == "paper_compile":
        compile_markdown = _stage_content(stage_outputs, "run_compile", "prepare_compile") or raw_markdown
        return format_paper_compile_report(project_label, prompt, compile_markdown, payload) if compile_markdown or stage_outputs else None

    if workflow_key == "paper_improvement":
        score_progress_markdown = raw_markdown or _stage_content(stage_outputs, "final_check", "revise_sections", "diagnose_draft")
        return (
    ...
    if workflow_key == "full_pipeline":
        ...
        return format_full_pipeline_report(project_label, prompt, handoff_markdown, payload) if handoff_markdown or stage_outputs else None

    if workflow_key == "auto_review_loop":
        return format_auto_review_loop_report(project_label, prompt, payload, raw_markdown) if raw_markdown or payload.get("iterations") or stage_outputs else None
    ...
    if workflow_key == "monitor_experiment":
        ...
        return (
            format_monitor_experiment_report(project_label, prompt, inspect_markdown, signals_markdown, payload)
    ...
    if workflow_key == "sync_workspace":
        ...
        return (
            format_sync_workspace_report(project_label, prompt, preview_markdown, sync_markdown, validation_markdown, payload)
            if preview_markdown or sync_markdown or validation_markdown or stage_outputs
            else None
        )

    if workflow_key == "custom_run":
        return format_custom_run_report(project_label, prompt, raw_markdown, payload) if raw_markdown or payload.get("execution_result") else None
...
def _extract_score_line(markdown: str) -> str | None:
    match = re.search(r"\bscore\b\s*[:：]\s*([0-9]+(?:\.[0-9]+)?(?:\s*/\s*10)?)", markdown, re.IGNORECASE)
    if match:
...
def _resolve_experiment_audit_payload(
    ...
    review_stage = stage_outputs.get("review_integrity") if isinstance(stage_outputs.get("review_integrity"), dict) else {}
