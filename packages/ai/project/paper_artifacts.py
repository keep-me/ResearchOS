...
_REVIEW_READY_PATTERN = re.compile(r"\b(ready for submission|accept|sufficient|ready)\b", re.IGNORECASE)
...
def build_figure_bundle(
    *,
    ...
    manifest = {
        ...
        "venue": venue,
        "artifacts": [
            {"id": "fig_training_curve", "relative_path": "figures/fig_training_curve.png", "kind": "figure"},
            {"id": "table_main_results", "relative_path": "figures/table_main_results.tex", "kind": "table"},
            {"id": "table_ablation", "relative_path": "figures/table_ablation.tex", "kind": "table"},
        ],
    }
    return {
...
def build_paper_write_bundle(
    *,
    ...
    intro = project_description.strip() or prompt.strip() or "This project investigates a research problem that still requires a sharper problem statement."
    related = (
        "Related work currently includes: "
        + (", ".join(paper_titles[:5]) if paper_titles else "TBD references from the linked paper library")
        + "."
    )
    method = _truncate(stage_markdown or "Method details should be refined from the current workflow output and implementation notes.", 1200)
    experiments = (
    ...
    sections = {
        "paper/sections/abstract.tex": "\n".join(["\\begin{abstract}", _latex_escape(abstract), "\\end{abstract}", ""]) + "\n",
...
def build_paper_improvement_bundle(
    *,
    ...
    return {
        "reports/paper-improvement-round1.md": (review_round_one.strip() or "待补充第一轮评审。").rstrip() + "\n",
        "reports/paper-improvement-round2.md": (review_round_two.strip() or "待补充第二轮评审。").rstrip() + "\n",
