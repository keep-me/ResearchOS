...
def build_deep_prompt(
    ...
    return (
        ...
        "要求：所有字段必须使用中文回答。\n"
        "证据说明：输入可能是跨全文选择的结构化证据包与阶段分析，不代表论文只到某一节；"
        "只有当某个具体判断确实缺少直接证据时，才写“证据不足”，不要把摘录跨度误判为证据不足，不要臆测或虚构。"
        "当前证据包也可能是为某个分析目标筛选出来的，某类细节未出现不代表原论文不存在。"
        "如果阶段分析与结构化证据包冲突，以结构化证据包为准。\n"
        f"详略级别：{normalized_detail}。\n"
        f"{_deep_detail_guidance(normalized_detail)}\n"
        f"论文标题: {title}\n"
        f"结构化证据与阶段分析: {extracted_pages}\n"
    )
...
def build_deep_focus_prompt(
    ...
    *,
    ...
    return (
        ...
        f"详略级别：{normalized_detail}。\n"
        f"{_deep_detail_guidance(normalized_detail)}\n"
        "证据说明：下面是跨全文选择的结构化证据包，不代表论文只到某一节；"
        "只有当某个具体判断确实缺少直接证据时，才写“证据不足”，不要把摘录边界误写成全文边界。"
        "当前证据包只服务于本轮焦点，未出现的其他细节不代表原文不存在。\n"
        f"论文标题: {title}\n"
        f"分析焦点: {focus_label}\n"
        f"任务要求: {instruction}\n"
        f"结构化证据包:\n{evidence_text}\n"
...
def build_reasoning_prompt(
    ...
    return (
        ...
        "## 证据使用要求\n"
        "以下提供的是跨全文选择的结构化证据包与已有分析，不代表论文只到某一节。\n"
        "除非证据明确显示缺页/截断，否则不要写“正文只覆盖到 Sec. x.x”。\n"
        "只有当某个具体判断确实找不到直接证据时，才写“证据不足”；"
        "不要把上下文过长、章节较多或引用分散误判为证据不足。\n"
        "如果同时给了“已有分析”，它们只是弱参考，用于术语对齐和快速回忆；一旦与结构化证据冲突，必须以结构化证据为准。\n"
        "结构化证据包也可能按当前任务做过筛选，某类细节未出现不代表原文没有。\n\n"
        "## 输出要求\n"
        "请输出严格的 JSON 对象：\n"
        "```json\n"
        "{\n"
