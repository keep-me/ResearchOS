...
def with_schema_version(
    value: dict[str, Any] | None, *, version: int = DEFAULT_SCHEMA_VERSION
) -> dict[str, Any]:
...
def versioned_list(
    items: list[dict[str, Any]] | None, *, version: int = DEFAULT_SCHEMA_VERSION
) -> list[dict[str, Any]]:
    return [
        with_schema_version(item, version=version) if isinstance(item, dict) else item
